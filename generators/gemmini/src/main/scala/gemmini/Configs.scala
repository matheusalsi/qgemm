...
object GemminiConfigs {
  ...
  val int4Config = defaultConfig.copy(
    ...
    spatialArrayInputType  = SInt(4.W),   // datapath do array em 4 bits
    spatialArrayWeightType = SInt(4.W),
    spatialArrayOutputType = SInt(12.W),  // 2*4 + log2(16)
    ...
    mvin_scale_args = Some(ScaleArguments(
      (t: SInt, f: Float) => {
        ...
        val overflow = rec_fn_to_in.io.intExceptionFlags(1)
        val maxsat8 = ((1 << (t.getWidth-1))-1).S
        val minsat8 = (-(1 << (t.getWidth-1))).S
        val sign = rawFloatFromRecFN(f.expWidth, f.sigWidth, rec_fn_to_in.io.in).sign
        val sat8 = Mux(sign, minsat8, maxsat8)
        val scaled = Mux(overflow, sat8, rec_fn_to_in.io.out.asTypeOf(t))
      ...
      4, Float(8, 24), 4,
      identity = "1.0",
      c_str = "({float y = ROUND_NEAR_EVEN((x) * (scale)); y > 7 ? 7 : (y < -8 ? -8 : (elem_t)y);})"
    )),
